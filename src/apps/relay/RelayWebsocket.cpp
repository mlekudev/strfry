...
void RelayServer::runWebsocket(ThreadPool<MsgWebsocket>::Thread &thr) {
    ...
    hubGroup->onConnection([&](uWS::WebSocket<uWS::SERVER> *ws, uWS::HttpRequest req) {
        ...
        if (cfg().relay__realIpHeader.size()) {
            ...
            // uWebSockets strips leading : characters, which interferes with IPv6 parsing.
            // Try parsing as-is first, then try restoring stripped colons.
            c->ipAddr = parseIP(header);
            if (c->ipAddr.empty()) c->ipAddr = parseIP("::" + header);
            if (c->ipAddr.empty()) c->ipAddr = parseIP(":" + header);
            if (c->ipAddr.empty()) LW << "Couldn't parse IP from header " << cfg().relay__realIpHeader << ": " << header;
        }
